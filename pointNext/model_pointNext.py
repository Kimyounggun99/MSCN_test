...
from openpoints.models.layers.conv import create_convblock1d, create_convblock2d
from openpoints.models.layers.activation import create_act
from openpoints.models.layers.local_aggregation import CHANNEL_MAP
from openpoints.models.layers.group import create_grouper, get_aggregation_feautres
...
class PointNextEncoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 nsample: int or List[int] = 16,
                 radius: float or List[float] = 0.15,
        ...
        radius_scaling = kwargs.get('radius_scaling', 1.5) # 2
    ...
    def forward_cls_feat(self, p0, f0=None):
        ...
        out= self.classifier(f0.squeeze(-1))
        return out

if __name__ == '__main__':
    device= torch.device('cuda:0')

    data = torch.rand(32, 3008, 3).to(device)
    
    print("===> testing pointNext ...")
    model = PointNextEncoder().to(device)
    
    out = model.forward_cls_feat(data)
    print(out.shape)
    breakpoint()